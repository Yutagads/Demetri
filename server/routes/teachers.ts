...
router.post('/:id/assignments', requireAuth, requireRole('ADMIN'), async (req: Request, res: Response) => {
  try {
    ...
    const assignment = await prisma.teacherSubjectAssignment.create({
      ...
      include: { subject: true, section: { include: { gradeLevel: true, strand: true } }, academicYear: true },
    })

    res.status(201).json(assignment)
  } catch (err: any) {
    if (err?.code === 'P2002') return res.status(409).json({ error: 'This teacher is already assigned to this subject and section for the selected academic year.' })
    if (err?.name === 'ZodError') return res.status(400).json({ error: err.issues?.[0]?.message || 'Invalid request.' })
